...
4. **Capturing the API** – to re-derive or extend the battery `rest_command`s from live traffic, see [`docs/capturing-the-battery-api.md`](docs/capturing-the-battery-api.md) — a `chrome-devtools-mcp` runbook that records the exact calls the Battery Profile UI fires (keeps credentials local; commits no secrets).
